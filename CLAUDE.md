...
- **Create the exercise** — Based on his request and knowledge in related technology, you create a short exercise for the user. The estimated duration can go up to 10 minutes and is usually around 5 minutes. Create a new folder in the exercises with the files necessary for the exercice. Name the folder following the convention `<tech>-<NNN>-<topic>` where `<tech>` is the lowercase technology name (e.g. `python`, `typescript`), `<NNN>` is a zero-padded 3-digit index incremented per technology (check existing folders to determine the next number), and `<topic>` is a short kebab-case label (e.g. `async-gather`, `rate-limiter`). Example: `python-004-decorators`, `typescript-004-mapped-types`. Once the exercice is ready, look at what time it is using a shell command, this is the **start time** of the user.
  - Exercises should be short tasks (up to 10 minutes) to be completed by the user. They target the area where the user want to improve and need to improve. For now the main focus is on programming languages.
    - **The time estimate is the time an experienced programmer would take** — read and understand the exercise, code it, plus a little debugging — with no doc searches. It is *not* an attempt to estimate how long this learner will take (that's impossible to predict, since it depends on research and understanding time). State it as such so the user reads it as a reference yardstick, not a personal deadline.
  ...
  - Exercises with a time estimate of more than 10 minutes should be exceptional, not including captsone exercises.
  - **Hints inside the exercise must never be the answer.** Any `Hint:` / comment written into the exercise files must point at a *concept, approach, or thing to look up* — never the literal solution. Forbidden: pasting the exact expression/function/type the user is meant to produce (e.g. writing `Awaited<ReturnType<typeof fn>>` when that IS the task). Allowed: naming the relevant tool ("this is the `Awaited` case — combine it with the type of the function's return"), describing the shape of the approach, or pointing to a pattern used elsewhere.
  - **Cover the edge cases in the test harness — don't only test the happy path.** Recent exercises have shipped with weak checks: a wrong implementation could still pass because the mock data never exercised the tricky part of the spec. When you write the runtime checks, add roughly **50% more cases** than the bare minimum needed to go green, and make the extra cases ones that *catch a plausible wrong answer*. Concretely: if the spec says "sorted", the data must contain ≥2 out-of-order elements so a missing `.sort()` actually fails; if it says "deduplicated", include duplicates; also cover empty inputs, single elements, boundaries, and every branch/variant the task names. Aim for: a half-correct solution should turn at least one check ❌.
  - **Self-review every exercise before handing it over.** After writing the exercise files, re-read every hint and comment and ask: "could the user pass this task by copy-pasting something I wrote?" If yes, rewrite that hint to be directional only. Then re-read the test harness and ask: "could a wrong implementation still pass because my mock data doesn't cover this case?" — if yes, add the missing case. Do this review silently as the last step before giving the user the start prompt.
